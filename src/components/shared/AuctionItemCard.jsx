...
const AuctionItemCard = ({data, auctionType, ...props}) => {
    ...
    const currentTopTime = parseFloat(data.toptime) - (Date.now() / 1000 - parseFloat(data.highestBidAt))
    ...
    return (
        <div
            className="col-span-1 flex flex-col text-center bg-white rounded-lg shadow divide-y divide-gray-200 hover:shadow-lg">
                    ...
                    <div>
                        ...
                        {(auctionType === 'FIXED-PRICE' && !data.settlementHash) && (
                            <button
                                ...
                                onClick={() => {
                                    props.isWalletConnected ? toggleModalVisible() : props.modelOpen()
                                }}
                                type="button"
                            >
                                Buy Now
                        ...
                        {(auctionType === 'TOP-TIME' && data.highestBid && currentTopTime > 0) && (
                    ...
                    <div>
                        <div className="mt-5 flex divide-x divide-gray-300">
                            <div className="w-0 flex-1 flex">
                                <a
                                    href=""
                                    onClick={() => window.open(`https://kovan.etherscan.io/tx/${data.creationHash}`)}
                            ...
                            {(auctionType === 'FIXED-PRICE' && data.settlementHash) && (
                                <div className="-ml-px w-0 flex-1 flex">
                                    <a
                                        href=""
                                        onClick={() => window.open(`https://kovan.etherscan.io/tx/${data.settlementHash}`)}
                                        className="relative -mr-px w-0 flex-1 inline-flex items-center justify-center py-2 text-sm text-gray-700 font-medium border border-transparent rounded-bl-lg hover:text-gray-500"
                                    >
                                        <ExternalLinkIcon className="w-5 h-5 text-gray-400" aria-hidden="true"/>
                            ...
                            <div className="-ml-px w-0 flex-1 flex">
                                <a
                                    href=""
                                    onClick={() => props.history.push(`/merchant/${data.creator}`)}
